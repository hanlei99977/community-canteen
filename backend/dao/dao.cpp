...
std::vector<Region> RegionDAO::getDistrictRegionsByCity(sql::Connection *conn, int city_region_id) {
    std::vector<Region> list;
    try {
        auto stmt = std::unique_ptr<sql::PreparedStatement>(
            conn->prepareStatement(
                "SELECT region_id, region_name, region_level, parent_id FROM region WHERE parent_id = ? AND region_level = '区级'"
            )
        );
        stmt->setInt(1, city_region_id);
        auto res = std::unique_ptr<sql::ResultSet>(stmt->executeQuery());
        while (res->next()) {
            Region r;
            r.setId(res->getInt("region_id"));
        ...
        std::cerr << "[RegionDAO::getDistrictRegionsByCity] Error: " << e.what() << std::endl;
    }
    return list;
}

std::vector<Region> RegionDAO::getCityRegions(sql::Connection *conn) {
    std::vector<Region> list;
    try {
        auto stmt = std::unique_ptr<sql::PreparedStatement>(
            conn->prepareStatement(
                "SELECT region_id, region_name, region_level, parent_id FROM region WHERE region_level = '市级'"
            )
        );
        auto res = std::unique_ptr<sql::ResultSet>(stmt->executeQuery());
        while (res->next()) {
        ...
        std::cerr << "[RegionDAO::getCityRegions] Error: " << e.what() << std::endl;
    }
    return list;
}

bool RegionDAO::isCityLevel(sql::Connection *conn, int region_id) {
    try {
        auto stmt = std::unique_ptr<sql::PreparedStatement>(
            conn->prepareStatement(
                "SELECT region_level FROM region WHERE region_id = ? AND region_level = '市级'"
            )
        );
        stmt->setInt(1, region_id);
        auto res = std::unique_ptr<sql::ResultSet>(stmt->executeQuery());
        ...
        std::cerr << "[RegionDAO::isCityLevel] Error: " << e.what() << std::endl;
    }
    return false;
}

int RegionDAO::getParentRegionId(sql::Connection *conn, int region_id) {
    try {
        auto stmt = std::unique_ptr<sql::PreparedStatement>(
            conn->prepareStatement(
                "SELECT parent_id FROM region WHERE region_id = ?"
            )
        );
        stmt->setInt(1, region_id);
        auto res = std::unique_ptr<sql::ResultSet>(stmt->executeQuery());
        ...
        std::cerr << "[RegionDAO::getParentRegionId] Error: " << e.what() << std::endl;
    }
    return -1;
}
...
DishDetailVO DishDAO::getDishDetail(sql::Connection *conn, int dish_id) {
    DishDetailVO detail;
    try {
        auto stmt = std::unique_ptr<sql::PreparedStatement>(
            conn->prepareStatement(
                "SELECT d.dish_id, d.name as dish_name, c.name as canteen_name, "
                "d.price, d.calories, d.nutrition_info "
                "FROM dish d "
        ...
        auto res = std::unique_ptr<sql::ResultSet>(stmt->executeQuery());
        if (res->next()) {
            detail.setDishId(res->getInt("dish_id"));
            detail.setDishName(res->getString("dish_name"));
            detail.setCanteenName(res->getString("canteen_name"));
            detail.setPrice(res->getDouble("price"));
            detail.setCalories(res->getInt("calories"));
            detail.setNutritionInfo(res->getString("nutrition_info"));
        }
        auto tagStmt = std::unique_ptr<sql::PreparedStatement>(
            conn->prepareStatement(
        ...
        tagStmt->setInt(1, dish_id);
        auto tagRes = std::unique_ptr<sql::ResultSet>(tagStmt->executeQuery());
        while (tagRes->next()) {
            detail.addTag(tagRes->getString("tag_name"));
        }
    } catch (const std::exception& e) {
        std::cerr << "[DishDAO::getDishDetail] Error: " << e.what() << std::endl;
    }
    return detail;
}
...
std::vector<std::shared_ptr<RecentOrderVO>> OrderDAO::getRecentOrders(sql::Connection *conn, int user_id, int order_for_user_id, int canteen_id, int limit) {
    std::vector<std::shared_ptr<RecentOrderVO>> result;
    try {
        auto stmt = std::unique_ptr<sql::PreparedStatement>(
            conn->prepareStatement(R"(
                SELECT order_id
        ...
        while (res->next()) {
            order_ids.push_back(res->getInt("order_id"));
        }
        ...
        std::cout << "获取多个最近订单失败: " << e.what() << std::endl;
        return result;
    }
}

std::vector<DishPurchaseRankingItem> OrderDAO::getDishPurchaseRanking(sql::Connection *conn, int user_id, const std::string& time_dimension) {
    std::vector<DishPurchaseRankingItem> result;
    try {
        ...
        auto stmt = std::unique_ptr<sql::PreparedStatement>(
            conn->prepareStatement(
                "SELECT d.dish_id, d.name as dish_name, c.name as canteen_name, SUM(oi.quantity) as quantity "
                "FROM orders o "
                "JOIN order_item oi ON o.order_id = oi.order_id "
                "JOIN dish d ON oi.dish_id = d.dish_id "
                "JOIN canteen c ON d.canteen_id = c.canteen_id "
        ...
        auto res = std::unique_ptr<sql::ResultSet>(stmt->executeQuery());
        while (res->next()) {
            DishPurchaseRankingItem item;
            item.setDishId(res->getInt("dish_id"));
            item.setDishName(res->getString("dish_name"));
            item.setCanteenName(res->getString("canteen_name"));
...
std::vector<DinerPreference> DinerPreferenceDAO::getUserPreferences(sql::Connection *conn, int user_id) {
    ...
    try {
        auto stmt = std::unique_ptr<sql::PreparedStatement>(
            conn->prepareStatement(
                "SELECT tag_id, score FROM diner_preference WHERE user_id = ?"
            )
        );
        stmt->setInt(1, user_id);
        auto res = std::unique_ptr<sql::ResultSet>(stmt->executeQuery());
